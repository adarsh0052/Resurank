...
def track_downloaded_file(file_path):
    ...
    print(
        f"File {os.path.basename(file_path)} will be automatically deleted in 10 minutes"
    )
...
def map_columns(df):
    ...
    for expected, aliases in column_aliases.items():
        ...
        if matched:
            ...
            raise ValueError(
                f"Could not find a column similar to '{expected}' in the Excel file."
            )
...
def extract_text_from_pdf(pdf_path):
    try:
        if pdf_path.startswith("http"):
            ...
            if "drive.google.com" in pdf_path:
                ...
                if file_id:
                    ...
                    try:
                        ...
                        print(
                            f"gdown download failed: {e}, trying alternative method..."
                        )
            ...
            if "pdf" in response.headers.get(
                "Content-Type", ""
            ).lower() or pdf_path.lower().endswith(".pdf"):
            ...
            for file_path in files_to_remove:
                try:
                    if os.path.exists(file_path):
                        os.unlink(file_path)
                        print(
                            f"Auto-deleted expired file: {os.path.basename(file_path)}"
                        )
                except Exception as e:
                    print(f"Error deleting expired file {file_path}: {e}")
